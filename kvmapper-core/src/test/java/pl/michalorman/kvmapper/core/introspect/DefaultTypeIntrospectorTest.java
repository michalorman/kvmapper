...
public class DefaultTypeIntrospectorTest {
    ...
    public void shouldOmitMissingWritableProperty(Class<?> type) {
        ...
        assertTrue(containsProperty(readableProperties, "property", String.class), "Should contain readable property of type String");

        assertTrue(result.getWritableProperties().isEmpty(), "Description should not have any writable properties.");
    }

    @ExceptProperties({ "booleanProperty", "doubleProperty" })
    ...
    @Test(description = "Should omit properties which are excluded via annotation.")
    public void shouldOmitExcludedProperties() {
        TypeDescription result = typeIntrospector.introspect(ExcludingProperties.class, config);

        Collection<ReadableProperty> readableProperties = result.getReadableProperties();
        assertEquals(readableProperties.size(), 1, "Should identify exactly 1 readable properties");
        assertTrue(containsProperty(readableProperties, "stringProperty", String.class), "Should contain readable property of type String");

        Collection<WritableProperty> writableProperties = result.getWritableProperties();
        assertEquals(writableProperties.size(), 1, "Should identify exactly 1 writable properties");
        assertTrue(containsProperty(writableProperties, "stringProperty", String.class), "Should contain writable property of type String");
    }

    @OnlyProperties({ "stringProperty", "booleanProperty" })
    public class IncludingProperties {
        private String stringProperty;
        private Boolean booleanProperty;
        private Double doubleProperty;
    ...
    @Test(description = "Should include only properties specified in annotation.")
    public void shouldIncludeOnlySpecifiedProperties() {
        TypeDescription result = typeIntrospector.introspect(IncludingProperties.class, config);
